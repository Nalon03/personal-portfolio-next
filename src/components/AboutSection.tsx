...
const BulletItem: React.FC<BulletItemProps> = ({ text, speed, shouldStart, onComplete, showStatic = false }) => {
  ...
  return (
    <li className={`flex items-start gap-2 min-h-[1.5rem] transition-opacity duration-200 ${showBullet ? 'opacity-100' : 'opacity-0'}`}>
...
const AboutSection: React.FC = () => {
  ...
  // Intersection Observer - only trigger animation on FIRST time becoming visible
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !hasPlayedRef.current) {
            // First time in view - start animation
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
        <header className="text-center mb-8 md:mb-10">
          ...
          <div 
            className="w-14 h-0.5 bg-gradient-to-r from-cyan-400 to-blue-500 mx-auto mt-3 rounded-full
        ...
        {/* ABOUT CONTENT */}
        <div 
          className="glass-card rounded-xl p-6 md:p-8 mb-12 opacity-0 animate-slide-up"
          style={{ animationDelay: '200ms', animationFillMode: 'forwards' }}
        >
          {/* Greeting */}
          <div className="mb-6 text-center">
            <span className="inline-block px-2.5 py-1 text-xs font-medium text-cyan-400 
                             bg-cyan-400/10 border border-cyan-400/20 rounded-full mb-3 font-sans">
              Full-Stack Developer
            </span>
            <h2 className="text-xl sm:text-2xl font-bold text-white font-display animate-pop-pulse">
        ...
        <div className="text-center">
          ...
          >
            What I <span className="gradient-text-cyan">Value</span>
          </h2>

          <div 
            className="glass-card rounded-xl p-6 md:p-8 opacity-0 animate-slide-up"
          ...
          >
            <div className="flex flex-wrap justify-center gap-2 mb-6">
              {['Clarity', 'Predictability', 'Performance', 'Stability'].map((value) => (
                <span 
                  key={value}
                  className="px-3 py-1.5 text-xs font-semibold text-cyan-300 
                             bg-cyan-400/10 border border-cyan-400/20 rounded-md font-sans"
                >
                  {value}
                </span>
              ))}
            ...
            <div className="w-10 h-0.5 bg-gradient-to-r from-cyan-400 to-blue-500 mx-auto mt-6 rounded-full" />
          </div>
        </div>
